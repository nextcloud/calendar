OC.L10N.register(
    "calendar",
    {
    "Calendar" : "Хуанли",
    "%s has published the calendar \"%s\"" : "%s дараахь хуанлийг нийтлэв: %s",
    "%s has published the calendar %s" : "%s дараахь хуанлийг нийтлэв: %s",
    "Hello," : "Сайн байна уу",
    "Click here to access it" : "Энд дарж хандана уу",
    "Too many redirects. Aborting ..." : "Хэтэрхий олон дамжилттай байна. Үйлдлийг цуцлав...",
    "Individual" : "Хувь хүний",
    "Group" : "Бүлэг",
    "Resource" : "Нөөц",
    "Room" : "Өрөө",
    "Unknown" : "Үл мэдэх",
    "Required" : "Заавал",
    "Optional" : "Сайн дурын",
    "Does not attend" : "Оролцоогүй",
    "You are in an unknown timezone ({tz}), falling back to UTC" : "Та үл мэдэх цагийн бүсд  ({tz}) байгаа тул UTC цагийн бүсийг ашиглана",
    "Please create a calendar first." : "Та эхлээд хуанли үүсгэнэ үү.",
    "New event" : "Шинэ үйл явдал",
    "Email has been sent." : "Цахим шууданг илгээв",
    "There was an issue while sending your email." : "Таны цахим шууданг илгээхэд алдаа гарлаа.",
    "group" : "бүлэг",
    "Details" : "Дэлгэрэнгүй",
    "Attendees" : "Оролцогчид",
    "Reminders" : "Сануулагууд",
    "Repeating" : "Давталт",
    "When shared show full event" : "Түгээсэн тохиолдолд үйл явдлыг бүтнээр нь харуул",
    "When shared show only busy" : "Түгээсэн тохиолдолд зөвхөн завгүй гэж харуул",
    "When shared hide this event" : "Түгээсэн тохиолдолд энэ үйл явдлыг нуу",
    "Confirmed" : "Баталгаажуулсан",
    "Tentative" : "Урьдчилсан",
    "Cancelled" : "Цуцалсан",
    "Please add a title." : "Та нэр өгнө үү.",
    "Please select a calendar." : "Та хуанли сонгоно уу.",
    "The event can not end before it starts." : "Үйл явдал эхлэхээсээ өмнө дуусах боломжгүй.",
    "Global" : "Глобал",
    "None" : "Аль нь ч биш",
    "Every day" : "Өдөр бүр",
    "Every week" : "Долоо хоног бүр",
    "Every month" : "Сар бүр",
    "Every year" : "Жил бүр",
    "never" : "Хэзээ ч үгүй",
    "after" : "дараа",
    "How about getting started by importing some calendars?" : "Хуанли импортлон оруулж ирснээр ажлаа эхлэх үү?",
    "At time of event" : "Үйл явдлын эхлэх үед",
    "5 minutes before" : "5 минутын өмнө",
    "10 minutes before" : "10 минутын өмнө",
    "15 minutes before" : "15 минутын өмнө",
    "30 minutes before" : "30 минутын өмнө",
    "1 hour before" : "1 цагийн өмнө",
    "2 hours before" : "2 цагийн өмнө",
    "Custom" : "Дурын",
    "Audio" : "Аудио",
    "Email" : "Цахим шуудан",
    "Pop-up" : "поп-ап",
    "sec" : "сек",
    "min" : "мин",
    "hours" : "цаг",
    "days" : "өдөр",
    "week" : "7 хоног",
    "before" : "өмнө",
    "start" : "эхлэх",
    "end" : "дуусах",
    "Editing reminders of unknown type not supported." : "Үл мэдэгдэх төрлийн санууллагыг засварлах боломжгүй.",
    "Week {number} of {year}" : "{year} оны {number}-р долоо хоног ",
    "Successfully imported" : "Амжилттай импортлов",
    "Partially imported, 1 failure" : "Хэсэгчлэн импортлов, 1 алдаа",
    "Partially imported, {n} failures" : "Хэсэгчлэн импортлов, {n} алдаа",
    "Audio alarm" : "Аудио сэрүүлэг",
    "Title of the Event" : "Үйл явдлын нэр",
    "Starts" : "Эхлэл",
    "Ends" : "Төгсгөл",
    "All day Event" : "Бүтэн өдрийн үйл явдал",
    "Delete" : "Устгах",
    "Cancel" : "Цуцлах",
    "More ..." : "Дэлгэрэнгүй ...",
    "Create" : "Үүсгэх",
    "Update" : "Шинэчлэх",
    "Close" : "Хаах",
    "Location" : "Байршил",
    "Description" : "Тайлбар",
    "Event status" : "Үйл явдлын статус",
    "Export" : "Экспорт",
    "Click on the link below to access it" : "Доорх холбоос дээр дарж хандана уу",
    "Day" : "Өдөр",
    "Week" : "Долоо хоног",
    "Month" : "Сар",
    "Today" : "Өнөөдөр",
    "Shared" : "Түгээсэн",
    "More" : "Дэлгэрэнгүй",
    "Edit" : "Засах",
    "Link" : "Холбоос",
    "Download" : "Татаж авах",
    "Share with users or groups" : "Бусад хэрэглэгч, бүлэгт түгээх",
    "can edit" : "засаж чадна",
    "Share link" : "Холбоос түгээх",
    "Public access" : "Нийтийн хандах эрх",
    "Email link to person" : "Холбоосыг цахим шуудангаар илгээх",
    "Send" : "Илгээх",
    "New Calendar" : "Шинэ хуанли",
    "Name" : "Нэр",
    "Go back" : "Буцах",
    "Go forward" : "Урагшлах",
    "Calendar does not exist" : "Хуанли байхгүй байна",
    "Edit event" : "Үйл явдлыг засварлах",
    "Delete Event" : "Үйл явдлыг устгах",
    "Save Event" : "Үйл явдлыг хадгалах",
    "Time" : "Цаг",
    "Type" : "Төрөл",
    "Add" : "Нэмэх"
},
"nplurals=2; plural=(n != 1);");
