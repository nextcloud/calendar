...
const selectedOption = computed<SelectOption | null>({
	get() {
		...
		if (internalSelectedOption.value) {
			return internalSelectedOption.value
		}
...
const daysOptions = computed<SelectOption[]>(() => {
	...
	for (let i = 0; i <= props.maxDays; i++) {
		options.push({
			id: i,
			label: i === 0 ? t('calendar', '0 days') : i === 1 ? t('calendar', '1 day') : t('calendar', '{count} days', { count: i }),
		})
	}
	return options
})
...
const hoursOptions = computed<SelectOption[]>(() => {
	...
	for (let i = 0; i <= props.maxHours; i++) {
		options.push({
			id: i,
			label: i === 0 ? t('calendar', '0 hours') : i === 1 ? t('calendar', '1 hour') : t('calendar', '{count} hours', { count: i }),
		})
	}
	return options
})
...
const minutesOptions = computed<SelectOption[]>(() => {
	...
	for (let i = 0; i < 60; i += props.minuteStep) {
		options.push({
			id: i,
			label: i === 0 ? t('calendar', '0 minutes') : i === 1 ? t('calendar', '1 minute') : t('calendar', '{count} minutes', { count: i }),
		})
	}
	return options
})
