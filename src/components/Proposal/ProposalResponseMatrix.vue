			...
			<table class="proposal-matrix__table" role="table">
				<thead>
					<tr class="proposal-matrix__header">
						...
						<th v-if="mode === 'participant'" scope="col" class="proposal-matrix__table-header-participant" colspan="2">
							{{ t('calendar', 'Select your meeting availability') }}
						</th>
						<th scope="col" v-for="participant in proposal.participants" :key="'head-' + participant.id"
						...
						<th v-if="mode === 'organizer'" scope="col" class="proposal-matrix__table-action-header">&nbsp;</th>
					</tr>
				</thead>
				<tbody>
...
import { Proposal, ProposalDate, ProposalResponse } from '@/models/proposals/proposals'
...
export default {
	...
	watch: {
		timezoneId(newZone) {
			if (newZone) {
				this.timezoneOffset = this.timezoneOffset(newZone)
			}
		},
	},
	...
	methods: {
		...
		participantVote(participantId, dateId) {
			...
			const vote = this.proposal.votes.find(v => 
