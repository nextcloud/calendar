...
<template>
  ...
  >
    <template v-slot:primary-actions>
      ...
      />
      <PropertyTitleTimePickerLoadingPlaceholder v-if="isLoading" />
    </template>
    ...
    <template v-slot:secondary-actions>
      <ActionLink
        ...
        icon="icon-download"
        :href="downloadURL"
      >{{ $t('calendar', 'Download') }}</ActionLink>
      ...
      >{{ $t('calendar', 'Delete') }}</ActionButton>
      <ActionButton
        v-if="canDelete && canCreateRecurrenceException"
        icon="icon-delete"
        @click="deleteAndLeave(false)"
      >{{ $t('calendar', 'Delete this occurrence') }}</ActionButton>
      <ActionButton
        v-if="canDelete && canCreateRecurrenceException"
        icon="icon-delete"
        @click="deleteAndLeave(true)"
      >{{ $t('calendar', 'Delete this and all future') }}</ActionButton>
    </template>
    ...
    >
      ...
      <div v-if="displayDetails" class="app-sidebar-tab__content">
        <AlarmList :calendar-object-instance="calendarObjectInstance" :is-read-only="isReadOnly" />
      </div>
      <SaveButtons
        v-if="showSaveButtons"
        class="app-sidebar-tab__buttons"
...
import PropertyTitleTimePickerLoadingPlaceholder from "../components/Editor/Properties/PropertyTitleTimePickerLoadingPlaceholder.vue";
...
export default {
  ...
  computed: {
    ...
    subTitle() {
      ...
      return moment(this.calendarObjectInstance.startDate)
        .locale(this.locale)
        .fromNow();
    },
    accessClass() {
